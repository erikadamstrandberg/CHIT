...
E_k_t1 = fft2c(E_incident*T_linear_phase_t1)
I_k_t1 = np.abs(E_k_t1)**2
I_k_t1 = eta_t1*I_k_t1/np.max(I_k_t1)
...
ax2.imshow(I_k_t, extent=extent_k, cmap='plasma')
...
E_plane_wave = np.ones(X.shape)
T_linear_phase = linear_phase_gradient_map(X, Y, n_air, lam0, anglex, angley)

E_k_space = fft2c(E_plane_wave)
I_k_space = np.abs(E_k_space)**2
I_k_space = I_k_space/np.max(I_k_space)

plt.imshow(I_k_space, extent=extent)
