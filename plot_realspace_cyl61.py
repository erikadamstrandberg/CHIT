...
class MEL_9000_x_images():
    ...
    def plot_image_and_y_cs(self, angle, n,  x_pos, integrate_over, plot_on):
...
plot_on = True
sodeg_x = image.plot_image_and_x_cs(angle_rad, n, y_pos, integrate_over, plot_on) #61 y
sodeg_y = image.plot_image_and_y_cs(angle_rad, n, x_pos, integrate_over, plot_on) #61 y
...
# image_paths = image_paths[0:3]

full_beam_x_cs = np.zeros(shape=(len(image_paths), len(sodeg_y)))
full_beam_y_cs = np.zeros(shape=(len(image_paths), len(sodeg_y)))

width_x = np.zeros(len(image_paths))
width_y = np.zeros(len(image_paths))
...
for i, im in enumerate(image_paths):
    ...
    image = MEL_9000_x_images(measurement_path)
    ...
    plot_on = False    
    full_beam_x_cs[i, :] = image.plot_image_and_x_cs(angle_rad, n, y_pos, integrate_over, plot_on)
    full_beam_y_cs[i, :] = image.plot_image_and_y_cs(angle_rad, n, x_pos, integrate_over, plot_on)
    
    full_beam_x_cs[i, :] = full_beam_x_cs[i, :]/np.max(full_beam_x_cs[i, :])
    full_beam_y_cs[i, :] = full_beam_y_cs[i, :]/np.max(full_beam_y_cs[i, :])
    
    index_x_mid = np.argmax(full_beam_x_cs[i,:])
    full_beam_x_cs_half = full_beam_x_cs[i, index_x_mid:]/np.max(full_beam_x_cs[i, index_x_mid:])
    index_1_over_e_x = np.argmin(np.abs(full_beam_x_cs_half - np.exp(-2)))
    width_x[i] = 2*index_1_over_e_x*image.pixel_size_k*1e6
    
    index_y_mid = np.argmax(full_beam_y_cs[i, :])
    full_beam_y_cs_half = full_beam_y_cs[i, index_y_mid:]/np.max(full_beam_y_cs[i, index_y_mid:])
    index_1_over_e_y = np.argmin(np.abs(full_beam_y_cs_half - np.exp(-2)))
    width_y[i] = 2*index_1_over_e_y*image.pixel_size_k*1e6
    

#%%

z_stack = np.arange(0, len(image_paths)*10 , 10)
extent = [image.x_axis.min()*1e6, image.x_axis.max()*1e6, z_stack.min(), z_stack.max()]

offset = np.zeros(len(image_paths))

plt.imshow(full_beam_x_cs, extent=extent, origin='lower')

#%%
plt.imshow(full_beam_y_cs, extent=extent, origin='lower')

#%%

plt.plot(z_stack, width_x)
plt.plot(z_stack, width_y)
