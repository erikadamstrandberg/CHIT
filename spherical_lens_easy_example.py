...
ms_name = 'spherical_lens_easy_example'
...
region_from = np.array([-12, -8, -2])
region_to   = np.array([-8,  -2, 0])

for i in range(len(region_from)):
    ...
    region_width   = (region_to[i] - region_from[i])
    region_radius  = (region_to[i] + region_from[i])/2
    ...
    supercell_ring_c = gf.components.ring(radius=region_radius, width=region_width, layer=(layer_dict[key]['layer'], layer_dict[key]['datatype']))
    supercell_ring_r = top << supercell_ring_c
